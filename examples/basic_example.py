...
def run_example():
    ...
    NUM_SAMPLES = 5
    STEP_SIZE = 0.1
    mc_prediction = mc.predict(filt.x, future_loading, n_samples = NUM_SAMPLES, dt=STEP_SIZE)
    print('ToE', mc_prediction.time_of_event.mean)

    # Step 3c: Analyze the results

    # Note: The results of a sample-based prediction can be accessed by sample, e.g.,
    states_sample_1 = mc_prediction.states[1]
    ...
    print('\tPortion between 3005.2 and 3005.6: ', mc_prediction.time_of_event.percentage_in_bounds([3005.2, 3005.6]))
    print('\tAssuming ground truth 3002.25: ', mc_prediction.time_of_event.metrics(ground_truth=3005.25))
    print('\tP(Success) if mission ends at 3002.25: ', prob_success(mc_prediction.time_of_event, 3005.25))

    # Plot state transition 
    # Here we will plot the states at t0, 25% to ToE, 50% to ToE, 75% to ToE, and ToE
    fig = mc_prediction.states.snapshot(0).plot_scatter(label = "t={} s".format(int(mc_prediction.times[0])))  # 0
    quarter_index = int(len(mc_prediction.times)/4)
    mc_prediction.states.snapshot(quarter_index).plot_scatter(fig = fig, label = "t={} s".format(int(mc_prediction.times[quarter_index])))  # 25%
    mc_prediction.states.snapshot(quarter_index*2).plot_scatter(fig = fig, label = "t={} s".format(int(mc_prediction.times[quarter_index*2])))  # 50%
    mc_prediction.states.snapshot(quarter_index*3).plot_scatter(fig = fig, label = "t={} s".format(int(mc_prediction.times[quarter_index*3])))  # 75%
    mc_prediction.states.snapshot(-1).plot_scatter(fig = fig, label = "t={} s".format(int(mc_prediction.times[-1])))  # 100%

    mc_prediction.time_of_event.plot_hist()
