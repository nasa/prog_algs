    ...
    with open('examples/data_const_load.csv', 'r') as f:
        ...
        for row in reader:
            ...
            if (step%PREDICTION_UPDATE_FREQ == 0):
                prediction_results = mc.predict(filt.x, future_loading, n_samples=NUM_SAMPLES, dt=TIME_STEP)
                m = prediction_results.time_of_event.metrics()
                ...
                rul_fig.canvas.draw()
                profile.add_prediction(t, prediction_results.time_of_event)
