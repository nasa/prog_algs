...
def run_example():
    ...
    STEP_SIZE = 0.1
    prediction_results = mc.predict(samples, future_loading, dt=STEP_SIZE, horizon=8)
    print("\nPredicted Time of Event:")
    pprint(prediction_results.time_of_event.metrics())  # Note this takes some time
    prediction_results.time_of_event.plot_hist(keys = 'impact')
    prediction_results.time_of_event.plot_hist(keys = 'falling')
